...
begin:
	...
	if(!proxytype) {
		...
		if(quic_connection_open(c, c->hostname, port)) {
			/* Fill in connection details BEFORE starting connection (needed for SNI) */
			c->last_ping_time = time(NULL);
			c->status.connecting = true;
			c->name = xstrdup(outgoing->node->name);  /* Set name BEFORE quic_connection_start */
			c->last_ping_time = now.tv_sec;
			
			/* Start QUIC connection */
			if(quic_connection_start(c)) {
				/* outmaclength/outcompression removed - QUIC only mode */
	...
	/* outcipher/outdigest/outmaclength/outcompression removed - QUIC only mode */
	c->last_ping_time = now.tv_sec;
...
void handle_new_unix_connection(void *data, int flags) {
	...
	c->socket = fd;
	c->last_ping_time = now.tv_sec;
	c->status.control = true;  /* Mark as control connection for Unix socket handling */
