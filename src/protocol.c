...
static bool (*request_handlers[])(connection_t *, const char *) = {
	id_h, NULL, NULL, NULL, ack_h,  /* metakey/challenge/chal_reply removed - QUIC only */
	NULL, NULL, termreq_h,
	ping_h, pong_h,
	add_subnet_h, del_subnet_h,
	add_edge_h, del_edge_h,
	NULL, NULL, NULL, NULL, control_h,  /* key_changed/req_key/ans_key/tcppacket removed */
	NULL, NULL, /* Not "real" requests (yet) */
	/* sptps_tcppacket removed */
	udp_info_h, mtu_info_h,
	/* Invitation protocol handlers */
	invite_join_h, invite_data_h, invite_csr_h, invite_cert_h,
};
...
static char (*request_name[]) = {
	...
	"REQ_PUBKEY", "ANS_PUBKEY", /* SPTPS_PACKET removed */ "UDP_INFO", "MTU_INFO",
	/* Invitation protocol */
	"INVITE_JOIN", "INVITE_DATA", "INVITE_CSR", "INVITE_CERT",
};
...
bool receive_request(connection_t *c, const char *request) {
	...
	if(reqno || *request == '0') {
		...
		if((c->allow_request != ALL) && (c->allow_request != reqno)) {
			/* Allow INVITE_JOIN as alternative to ID for initial connection */
			if(!(c->allow_request == ID && reqno == INVITE_JOIN)) {
