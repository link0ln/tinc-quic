...
#ifdef HAVE_MSQUIC
			/* For QUIC connections (protocol 17.7+), terminate if quic_context is NULL */
			/* This means the QUIC connection has shut down and needs cleanup */
			if(c->protocol_minor >= 7 && !c->quic_context) {
				/* Safety check: only terminate if connection has an edge */
				/* Connections without edges are terminated elsewhere (line 240) */
				if(!c->edge) {
					logger(DEBUG_CONNECTIONS, LOG_DEBUG, "QUIC connection to %s (%s) has no edge, skipping termination",
					       c->name ? c->name : "unknown", c->hostname ? c->hostname : "unknown");
					continue;
				}

				logger(DEBUG_CONNECTIONS, LOG_INFO, "QUIC connection to %s (%s) is dead (edge=%p, node=%p), terminating to trigger reconnection",
				       c->name ? c->name : "unknown", c->hostname ? c->hostname : "unknown",
				       (void *)c->edge, (void *)c->node);
				terminate_connection(c, c->edge);
				continue;
			}
#endif
