...
bool control_h(connection_t *c, const char *request) {
	...
	case REQ_PCAP:
		/* outmaclength removed - QUIC only mode */
		c->status.pcap = true;
		pcap = true;
		return true;

	case REQ_LOG:
		/* outcompression removed - QUIC only mode */
		c->status.log = true;
		logcontrol = true;
		return true;
	...
	case REQ_INVITE: {
		...
		if(sscanf(request, "%*d %*d " MAX_STRING, name) != 1) {
			return control_return(c, REQ_INVITE, -1);
		}
		...
		if(result == 0 && url) {
			send_request(c, "%d %d %d %s", CONTROL, REQ_INVITE, 0, url);
			free(url);
			return true;
		} else {
			return control_return(c, REQ_INVITE, result);
		}
	}
	...
	case REQ_JOIN: {
		...
		if(sscanf(request, "%*d %*d " MAX_STRING " " MAX_STRING " " MAX_STRING,
		          host, port, token) != 3) {
			return control_return(c, REQ_JOIN, -1);
		}

		/* Start join process - this will send status updates */
		int result = process_join_request(c, host, port, token);
		return result == 0;
	}
