...
#include "crypto.h"
/* ecdsagen.h/rsagen.h removed - QUIC only mode */
#include "fsck.h"
#include "info.h"
#include "names.h"
#include "utils.h"
...
/* Stub for netutl.c when linked with tinc (not tincd) */
...
/*
  ...
  them in.
*/
/* ed25519_keygen removed - QUIC only mode */
...
/*
  ...
  them in.
*/
/* rsa_keygen removed - QUIC only mode */
#endif
...
/* get_pubkey removed - QUIC only mode */
...
const var_t variables[] = {
	...
	{"Ed25519PrivateKeyFile", VAR_SERVER | VAR_OBSOLETE},
	...
	{"PrivateKeyFile", VAR_SERVER | VAR_OBSOLETE},
	...
	{"Cipher", VAR_SERVER | VAR_HOST | VAR_OBSOLETE},
	{"ClampMSS", VAR_SERVER | VAR_HOST | VAR_SAFE},
	{"Compression", VAR_SERVER | VAR_HOST | VAR_SAFE},
	{"Digest", VAR_SERVER | VAR_HOST | VAR_OBSOLETE},
	{"Ed25519PublicKey", VAR_HOST | VAR_OBSOLETE},
	{"Ed25519PublicKeyFile", VAR_SERVER | VAR_HOST | VAR_OBSOLETE},
	{"IndirectData", VAR_SERVER | VAR_HOST | VAR_SAFE},
	{"MACLength", VAR_SERVER | VAR_HOST | VAR_OBSOLETE},
...
static int cmd_init(int argc, char *argv[]) {
	...
	/* Generate full tinc.conf with all options documented */
	if(!generate_default_config(tinc_conf, name)) {
	...
	fprintf(stderr, "\nNext steps:\n");
	fprintf(stderr, "  1. Edit %s to configure VPNAddress and other options\n", tinc_conf);
	fprintf(stderr, "  2. For DPI evasion, set up TLS certificates (QUICCertFile/QUICKeyFile)\n");
	fprintf(stderr, "  3. Add ConnectTo entries for nodes to connect to\n");
	fprintf(stderr, "  4. Start the daemon: tincd -n %s\n", netname ? netname : name);

	check_port(name);

	/* tinc-up script creation removed - use VPNAddress in tinc.conf instead */
...
static int cmd_generate_keys(int argc, char *argv[]) {
	(void)argc;
	(void)argv;
	fprintf(stderr, "Legacy key generation not supported in QUIC mode. Use OpenSSL to generate TLS certificates.\n");
	return 1;
}

/* cmd_generate_rsa_keys removed - QUIC only mode */

/* cmd_generate_ed25519_keys removed - QUIC only mode */
...
static int cmd_sign(int argc, char *argv[]) {
	(void)argc;
	(void)argv;
	fprintf(stderr, "Error: ECDSA signing not supported in QUIC-only mode\n");
	return 1;
}
...
static int cmd_verify(int argc, char *argv[]) {
	(void)argc;
	(void)argv;
	fprintf(stderr, "Error: ECDSA signature verification not supported in QUIC-only mode\n");
	return 1;
}

static int cmd_invite(int argc, char *argv[]) {
	if(argc != 2) {
		fprintf(stderr, "Usage: %s invite <name>\n", program_name);
		return 1;
	}
	...
	sendline(fd, "%d %d %s", CONTROL, REQ_INVITE, argv[1]);

	char line[4096];
	if(!recvline(fd, line, sizeof(line))) {
		fprintf(stderr, "Error getting response from tincd.\n");
		return 1;
	}

	int code, req;
	char url[4096] = {0};
	if(sscanf(line, "%d %d %d %4000s", &code, &req, &code, url) < 3) {
		fprintf(stderr, "Error parsing response from tincd.\n");
		return 1;
	}
	...
	if(code != 0) {
		...
		switch(code) {
			...
			case -2: msg = "Node already exists"; break;
			case -3: msg = "No CA authority (ca-key.pem missing)"; break;
			case -4: msg = "Failed to create invitation"; break;
...
static int cmd_join(int argc, char *argv[]) {
	if(argc != 2) {
		fprintf(stderr, "Usage: %s join <invitation-url>\n", program_name);
		return 1;
	}

	/* Parse URL: https://host:port/invite/token */
	...
	if(!slash) {
		fprintf(stderr, "Invalid invitation URL: missing path\n");
		return 1;
	}

	size_t hostport_len = slash - url;
	if(hostport_len >= sizeof(host)) {
		fprintf(stderr, "Invalid invitation URL: host too long\n");
		return 1;
	}

	char hostport[256];
	strncpy(hostport, url, hostport_len);
	...
	/* Parse /invite/<token> */
	if(strncmp(slash, "/invite/", 8) == 0) {
		strncpy(token, slash + 8, sizeof(token) - 1);
	} else {
		fprintf(stderr, "Invalid invitation URL: expected /invite/<token>\n");
		return 1;
	}

	if(!token[0]) {
		fprintf(stderr, "Invalid invitation URL: missing token\n");
		return 1;
	}

	printf("Joining network via %s:%s...\n", host, port);

	if(!connect_tincd(true)) {
		return 1;
	}

	sendline(fd, "%d %d %s %s %s", CONTROL, REQ_JOIN, host, port, token);
	...
	while(recvline(fd, line, sizeof(line))) {
		...
		if(sscanf(line, "%d %d %d %4000[^\n]", &code, &req, &status, message) >= 3) {
			if(status < 0) {
				fprintf(stderr, "Error: %s\n", message[0] ? message : "Join failed");
				return 1;
			} else if(status == 0) {
	...
	fprintf(stderr, "Connection lost during join\n");
	return 1;
}
...
static const struct {
	...
	/* generate-rsa-keys removed - QUIC only mode */
	/* generate-ed25519-keys removed - QUIC only mode */
	{"help", cmd_help, false},
