...
#include "ipv6.h"
/* cipher.h/digest.h removed - QUIC only mode */
#include "event.h"
...
/* MAXSIZE is the maximum size of an encapsulated packet: MTU + seqno + srcid + dstid + compressor overhead
   In QUIC mode, encryption/MAC is handled by QUIC layer, so no cipher block or digest overhead */
#define MAXSIZE (MTU + 4 + sizeof(node_id_t) + sizeof(node_id_t) + MTU/64 + 20)

/* MAXBUFSIZE is the maximum size of a request
   Increased from 2048 to 65536 to support INVITE_DATA with many hosts (each ~500 bytes JSON) */
#define MAXBUFSIZE ((MAXSIZE > 65536 ? MAXSIZE : 65536) + 128)
...
/* send_sptps_data removed - QUIC only mode */
...
/* receive_tcppacket_sptps removed - QUIC only mode */
...
/* Legacy ECDSA/RSA key reading removed - QUIC only mode */
