...
const sockaddr_t *get_recent_address(address_cache_t *cache) {
	...
	// First, check hosts.json database for addresses
	if(cache->json_addr_idx == -1) {
		cache->json_addr_idx = 0;  // Start iterating
	}

	if(hosts_db && cache->json_addr_idx >= 0) {
		host_entry_t *host = hosts_db_find(cache->node->name);
		if(host && cache->json_addr_idx < host->address_count) {
			// Get address from hosts.json
			const char *addr = host->addresses[cache->json_addr_idx];
			cache->json_addr_idx++;
			...
			cache->aip = cache->ai = str2addrinfo(addr, port_str, SOCK_STREAM);

			if(cache->ai && cache->aip) {
				sockaddr_t *sa = (sockaddr_t *)cache->aip->ai_addr;
				cache->aip = cache->aip->ai_next;
				return sa;
			}
		} else {
			// No more JSON addresses, mark as exhausted
			cache->json_addr_idx = -2;
		}
	}

	// Fallback: check if there are any known Address statements from host files
