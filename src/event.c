...
void io_add(io_t *io, io_cb_t cb, void *data, int fd, int flags) {
	logger(DEBUG_ALWAYS, LOG_INFO, "io_add called: io=%p, cb=%p, fd=%d, flags=%d, io->cb=%p",
	       (void *)io, (void *)cb, fd, flags, (void *)io->cb);

	if(io->cb) {
		logger(DEBUG_ALWAYS, LOG_WARNING, "io_add: io->cb already set (%p), ignoring registration for fd=%d",
		       (void *)io->cb, fd);
		return;
	}
...
bool event_loop(void) {
	...
	while(running) {
		...
		if(loop_counter++ % 100 == 0) {
			logger(DEBUG_TRAFFIC, LOG_DEBUG, "event_loop: io_count=%d, max_fd=%d", io_count, fds - 1);
		}

		int n = select(fds, &readable, &writable, NULL, tv);

		if(n > 0 && loop_counter % 10 == 0) {
			logger(DEBUG_TRAFFIC, LOG_DEBUG, "select returned %d ready fds", n);
		}
