...
static bool read_packet(vpn_packet_t *packet) {
	...
	case DEVICE_TYPE_TUN:
		...
		/* TUN device includes 4-byte PI header (without IFF_NO_PI):
		   Bytes 10-11: flags
		   Bytes 12-13: protocol (0x0800=IPv4, 0x86dd=IPv6)
		   Bytes 14+: actual IP packet
		   We use the protocol field from PI header which is set by kernel */
		uint16_t tun_proto = (DATA(packet)[12] << 8) | DATA(packet)[13];

		logger(DEBUG_TRAFFIC, LOG_DEBUG, "Read %d bytes from TUN, PI header: %02x %02x %02x %02x, proto=0x%04x",
		       inlen, DATA(packet)[10], DATA(packet)[11], DATA(packet)[12], DATA(packet)[13], tun_proto);
		logger(DEBUG_TRAFFIC, LOG_DEBUG, "  Before memset - packet->offset=%d, packet->data[24-25]=%02x %02x",
		       packet->offset, packet->data[24], packet->data[25]);

		memset(DATA(packet), 0, 12);

		/* Copy protocol from PI header to Ethernet type field (bytes 12-13) */
		DATA(packet)[12] = (tun_proto >> 8) & 0xff;
		DATA(packet)[13] = tun_proto & 0xff;

		logger(DEBUG_TRAFFIC, LOG_DEBUG, "  After setting eth type - DATA(packet)[12-13]=%02x %02x, packet->data[24-25]=%02x %02x",
		       DATA(packet)[12], DATA(packet)[13], packet->data[24], packet->data[25]);

		/* Validate it's a known protocol */
		if(tun_proto != 0x0800 && tun_proto != 0x86dd) {
			logger(DEBUG_TRAFFIC, LOG_ERR, "Unknown protocol 0x%04x from %s",
			       tun_proto, device_info);
			return false;
		}
